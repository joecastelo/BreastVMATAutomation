...
namespace BreastVMATAutomation.UI
{
    internal class BreastGeometryTool
    {
        ...
        public BreastGeometryTool(ExternalPlanSetup plan, IEnumerable<Structure> selectedTargets,
            ArcModel selectedSetup, LINACDefaultCalculation linacDefaultCalculation, NonRDSOptions nonRDSOptions)
        {
            Plan = plan;
            SelectedTargets = selectedTargets;
            SelectedSetup = selectedSetup;
            LinacDefaultCalculation = linacDefaultCalculation;
        ...
        {
            Isocenter = SelectIsocenter.ChangeIsocenter(Plan, SelectedTargets.First());
        ...
        public void CreateArcsBasedOnModel()
        {
            ...
            try
            {
                ...
                foreach (var wise in Enumerable.Range(0, SelectedSetup.NbPartialArcs / 2))
                {
                    if (wise % 2 == 0)
                    {
                        var inner = GetInnerBeam(bestInner, 5 * (1 + wise));
                        var outer = GetOuterBeam(10, bestOuter, 360 - (wise + 1) * 5);
        ...
        private Beam GetNodeArc()
        {
            ...
            var mlcBeam = Plan.AddMLCArcBeam(machineParameters, mlc_leaf_pos, new VRect<double>(), 85, 320, 179, GantryDirection.Clockwise, 0, Isocenter);
            mlcBeam.FitCollimatorToStructure(new FitToStructureMargins(10, 10, 10, 10), SelectedTargets.First(), true, true, false);
        ...
        private Beam ReverseArc(Beam beam)
        {
            ...
            if (RDS)
            {
                return Plan.AddVMATBeamForFixedJaws(machineParameters, MeterSetWeight(), colAngle, finalAngle, initAngle, GantryDirection.CounterClockwise, 0, isocenter);
            }
        ...
        private int CalculateBeamBestInner()
        {
            ...
            if (RDS)
            {
                return 310 - SelectedSetup.InnerAngleMargin;
            }
            try
            {
                for (int i = 0; i < 12; i++)
                {
                    ...
                    beam.FitCollimatorToStructure(new FitToStructureMargins(0, 7, 25, 7), SelectedTargets.First(), true, true, false);
                    if (NonRDSOptions.AvoidEntryCLBreast)
                    {
                        ...
                        var npoints = BeamParameterCalculations.PointsInsideJaw(countPoints, beam);
                        if (npoints < 20)
                        {
                            var body = Plan.StructureSet.Structures.First(E => E.DicomType == "EXTERNAL");
                            innerAnglesAreas.Add(initAngle + (i * 5),
                                BeamParameterCalculations.PercentageAreaComparingBody(beam, body, SelectedTargets.First()));
                        }
        ...
        private int CalculateBeamBestOuter()
        {
            ...
            try
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    ExternalBeamMachineParameters parameters = GetStaticParameters();
                    var beam = Plan.AddStaticBeam(parameters, new VRect<double>(), 0, initAngle + (i * 5), 0, Isocenter);
                    beam.FitCollimatorToStructure(new FitToStructureMargins(30, 7, 0, 7), SelectedTargets.First(), true, true, false);
        ...
        private Beam GetOuterBeam(int innerLastAngle, int outerGantryAngle,int collimatorAngle)
        {
            ...
            var machineParameters = Parameters;
            var isocenter = Isocenter;
            var mlcBeam = Plan.AddMLCArcBeam(machineParameters, mlc_leaf_pos, new VRect<double>(), collimatorAngle, outerGantryAngle, outerGantryAngle+10, GantryDirection.Clockwise, 0, isocenter);
            mlcBeam.FitCollimatorToStructure(new FitToStructureMargins(35, 7, 0, 7), SelectedTargets.First(), true, true, false);
            var editparams = mlcBeam.GetEditableParameters();
            var jawPost = editparams.ControlPoints.First().JawPositions;
            Plan.RemoveBeam(mlcBeam);
            var finalAngle = outerGantryAngle + SelectedSetup.OuterAngleMargin > 179 ? 179 : outerGantryAngle + SelectedSetup.OuterAngleMargin;
            mlcBeam = Plan.AddMLCArcBeam(machineParameters, mlc_leaf_pos, jawPost, collimatorAngle, innerLastAngle + SelectedSetup.GapSectorAngle, 
                finalAngle, GantryDirection.Clockwise, 0, isocenter);
        ...
        private Beam GetInnerBeam(int gantryAngle, int collimatorAngle)
        {
            ...
            var machineParameters = Parameters;
            var mlcBeam = Plan.AddMLCArcBeam(machineParameters, mlc_leaf_pos, new VRect<double>(), collimatorAngle, gantryAngle, 10, GantryDirection.Clockwise, 0, Isocenter);
            mlcBeam.FitCollimatorToStructure(new FitToStructureMargins(0, 7, 35, 7), SelectedTargets.First(), true, true, false);
            if (NonRDSOptions.XLessThan15)
            {
                var jawSum = BeamParameterCalculations.GetXColimatorSize(mlcBeam);
                if (jawSum > 150)
                {
                    mlcBeam = LimitJawSizeX2(mlcBeam);
                }
            }
            return mlcBeam;
        }
        ...
        {
            var machineParameters = Parameters;
            var mlcBeam = Plan.AddVMATBeamForFixedJaws(machineParameters, MeterSetWeight() ,  collimatorAngle, gantryAngle, 10, GantryDirection.Clockwise, 0, Isocenter);
            return mlcBeam;
        }
        private Beam GetOuterBeamRDS(int innerLastAngle, int outerGantryAngle, int collimatorAngle)
        {
            ...
            var mlcBeam = Plan.AddVMATBeamForFixedJaws(machineParameters, MeterSetWeight(), collimatorAngle,  innerLastAngle + SelectedSetup.GapSectorAngle,
                finalAngle, GantryDirection.Clockwise, 0, Isocenter);
            return mlcBeam;
        }
